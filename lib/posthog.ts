...
posthog.init(${JSON.stringify(key)},{api_host:"https://eu.i.posthog.com",defaults:"2026-05-30",capture_pageview:true,capture_pageleave:true,autocapture:true,disable_session_recording:false,loaded:function(ph){ph.register(Object.assign({site:window.location.hostname},(function(){var q=new URLSearchParams(window.location.search),u={};["utm_source","utm_medium","utm_campaign"].forEach(function(n){var v=q.get(n);if(v)u[n]=v;});return u;})()));}});
...
(function(){
  ...
  function check(){
    ...
    var percent=scrollable<=0?100:((window.scrollY||doc.scrollTop)/scrollable)*100;
    marks.forEach(function(mark){
      if(!fired[mark]&&percent>=mark-(mark===100?1:0)){fired[mark]=true;posthog.capture("scroll_depth",{percent:mark,site:window.location.hostname});}
    });
  }
